...
*/
for (let i = 0; i < starWarsCharacters.lehght; i++) {
...
let crewMass = 0;
let g = 0;
while (g < starWarsCharacters.length) {
  crewMass += starWarsCharacters[g].mass;
  g++;
}
...
for (let i = 0; i < starWarsCharacters; i++) {
  for (let j = 0; j < characters.length; i++) {
    if (femaleCharacters[i].name === characters[j]) {
      characters.slice(j, 1);
    }
  }
}
console.log(characters);
...
*/
console.log(starWarsCharacters[Math.ceil(Math.random() * 10) - 8]);
console.log(
  "Prendo un elemento casuale di starWarsCharacterrs e lo visualizzo diversamente ogni volta"
);
